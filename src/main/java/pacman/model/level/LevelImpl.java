...
public class LevelImpl implements Level {
    ...
    // For frightened mode counting
    private int ghostEatenCount = 0;
    private static final int[] ghostPoints = {200, 400, 800, 1600};
    ...
    public void tick() {
        if (this.gameState != GameState.IN_PROGRESS) {
            ...
            if (isFrightened) {
                if (tickCount >= modeLengths.get(GhostMode.FRIGHTENED) * FPS) {
                    isFrightened = false;
                    for (Ghost ghost : ghosts) {
                        ghost.handleFrightenedDone();
                    }
                    tickCount = 0;
                }
            }
            else if (tickCount == modeLengths.get(currentGhostMode) * FPS) {
                // update ghost mode using state pattern
                for (Ghost ghost : this.ghosts) {
                    ghost.handleStateChange();
                }
                currentGhostMode = GhostMode.getNextGhostMode(currentGhostMode);
                tickCount = 0;
            }
    ...
    @Override
    public void handleGhostEaten() {
        if (ghostEatenCount < ghostPoints.length) {
            points += ghostPoints[ghostEatenCount];
            notifyObserversWithScoreChange(ghostPoints[ghostEatenCount]);
            ghostEatenCount++;
        }
    }
